...
! Mass matrix assembly (with another number of integration points)
    ...
    do i  = 1,nels
        ...
        call asemcoo2(g,mv0,mm,nnz,ia,ja,isys)
    end do
! Sparse form
    call coicsr(1,neq,mv0(1:nnz),ia(1:nnz),ja(1:nnz))
    call clncsr2(3,1,neq,mv0,ia,ja,indu)
    nnz = ia(neq+1) - 1
    allocate(kv(nnz))
    allocate(cv(nnz))
    allocate(mv(nnz))
...
! Time stepping loop
    ...
    do it = 1,nstep
        ...
        u20 = a0*d + a2*v + a4*a
        u10 = a1*d + a3*v + a5*a
        call amux(mv,ia,ja,u20,vc)
        call amux(cv,ia,ja,u10,kd)
        rt = rt + vc + kd
        ! Iteration loop
        do iter = 1,miter
            ! Update stresses
            ...
            u10 = a0*u0
            u20 = a1*u0
            call amux(mv,ia,ja,u10,vc)
            call amux(cv,ia,ja,u20,kd)
            kd = rt - vc - kd - eload
            kp = kv + mv*a0 + cv*a1
        ...
        u10 = a1*(u0 - d) - a3*v - a5*a
        u20 = a0*(u0 - d) - a2*v - a4*a
        d = u0
        v = u10
        a = u20
        ! step paraview
        out = zero
        out(1:) = d
        write(chtime,'(es16.10)') time
        open(15,file = fdat(1:nlen)//'_T'//chtime//'_P0.vtu')
        call step_pvd(15, g_coord, g_num, out, nf)
        close(15)
        tv(it) = time
    end do
    ...
    ! Paraview
    open(1,file =  fdat(1:nlen)//'.pvd')
    model = 'soil2'
    call main_pvd(1, trim(model), tv)
    close(1)

program pc1
    ...
    integer(ikd) :: ns
    integer(ikd) :: nnz
    integer(ikd), allocatable :: ia(:)
    integer(ikd), allocatable :: ja(:)
    integer(ikd), allocatable :: indu(:)
    ...
    do iel = 1,nels
        ...
        call asemcoo1(g,kv,km,nnz,ia,ja,isys)
    ...
    close(10)
    call coicsr(1,neq,kv(1:nnz),ia(1:nnz),ja(1:nnz))
    call clncsr(3,1,neq,kv,ia,ja,indu)
    nnz = ia(neq+1) - 1
    nth = 2
    call superlu_solver(neq,nnz,kv,ia,ja,loads(1:))
    loads(0) = zero
    if (ndim.eq.3) then
